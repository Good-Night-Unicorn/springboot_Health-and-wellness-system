	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'Ξ'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/zixunjilu?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">咨询编号</div>
						<div class="text "  >{{detail.zixunbianhao}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">用药意见</div>
						<div class="text "  >{{detail.yongyaoyijian}}</div>
					</div>
					<div class="item">
						<div class="lable">医生建议</div>
						<div class="text "  >{{detail.yishengjianyi}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('zixunjilu','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('zixunjilu','删除')" @click="delClick">删除</el-button>
					</div>
				</div>
			</div>
		...
		methods: {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/zixunjilu', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 删除
			async delClick(){
				await this.$confirm('是否删除此咨询记录？') .then(_ => {
