		...
		<div class="list-preview">
			...
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<el-form-item class="list-item">
					...
					<el-input v-model="formSearch.shipinjianjie" placeholder="视频简介" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<el-button class="list-search-btn" v-if=" true " type="primary" @click="getList(1, curFenlei)">
					<i class="el-icon-search"></i>
					查询
				</el-button>
				<el-button class="list-add-btn" v-if="btnAuth('jiankangshipin','新增')" type="primary" @click="add('/index/jiankangshipinAdd')">
			...
			<div class="list">
				...
				<div class="list2 index-pv1">
					<div v-for="(item, index) in dataList" :key="index" class="list-item animation-box" @click.stop="toDetail(item)">
						<div class="img">
							<img @click.stop="imgPreView(item.shipinfengmian)" v-if="item.shipinfengmian && item.shipinfengmian.substr(0,4)=='http'&&item.shipinfengmian.split(',w').length>1" :src="item.shipinfengmian" class="image" />
							<img @click.stop="imgPreView(item.shipinfengmian.split(',')[0])" v-else-if="item.shipinfengmian && item.shipinfengmian.substr(0,4)=='http'" :src="item.shipinfengmian.split(',')[0]" class="image" />
							<img @click.stop="imgPreView(baseUrl + (item.shipinfengmian?item.shipinfengmian.split(',')[0]:''))" v-else :src="baseUrl + (item.shipinfengmian?item.shipinfengmian.split(',')[0]:'')" class="image" />
						</div>
						<div class="item-info">
							<div class="name">{{item.shipinmingcheng}}</div>
...
<script>
	export default {
		...
		methods: {
			...
			async getFenlei() {
				await this.$http.get('option/shipinleixing/shipinleixing').then(res => {
					if (res.data.code == 0) {
			...
			getList(page, fenlei, ref = '') {
				...
				if (this.formSearch.shipinjianjie != '') searchWhere.shipinjianjie = '%' + this.formSearch.shipinjianjie + '%';
				if (this.curFenlei != '全部') searchWhere.shipinleixing = this.curFenlei;
				let user = JSON.parse(localStorage.getItem('sessionForm'))
				if (this.sortType) searchWhere.sort = this.sortType
				if (this.sortOrder) searchWhere.order = this.sortOrder
				this.$http.get(`jiankangshipin/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/jiankangshipinDetail', query: params});
			},
