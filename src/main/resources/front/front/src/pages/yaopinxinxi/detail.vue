		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">药品分类</div>
						<div class="text "  >{{detail.yaopinfenlei}}</div>
					</div>
					<div class="item">
						<div class="lable">剂型</div>
						<div class="text "  >{{detail.jixing}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						...
						<div class="text "  >{{detail.danjia}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('yaopinxinxi','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('yaopinxinxi','删除')" @click="delClick">删除</el-button>
					</div>
				</div>
			</div>
			...
			<el-tabs class="detail-tabs" v-model="activeName" type="border-card" v-if="tabsNum>0" >
				<el-tab-pane label="药品说明" name="first">
					<div class="ql-snow ql-editor" v-html="detail.yaopinshuoming"></div>
				</el-tab-pane>
			</el-tabs>
...
<script>
	...
	export default {
		...
		methods: {
			init() {
				...
				this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(res => {
					if (res.data.code == 0) {
						...
						if(this.detail.yaopintupian) {
							this.detailBanner = this.detail.yaopintupian.split(",w").length>1?[this.detail.yaopintupian]:this.detail.yaopintupian.split(',');
						}
						this.$forceUpdate();
			...
			// 删除
			async delClick(){
				await this.$confirm('是否删除此药品信息？') .then(_ => {
