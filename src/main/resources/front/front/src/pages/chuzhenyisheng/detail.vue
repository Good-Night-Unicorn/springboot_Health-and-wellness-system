	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'Ξ'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/chuzhenyisheng?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">医生账号</div>
						<div class="text "  >{{detail.yishengzhanghao}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">上班时间</div>
						<div class="text "  >{{detail.shangbanshijian}}</div>
					</div>
					<div class="item">
						<div class="lable">出诊日期</div>
						<div class="text "  >{{detail.chuzhenriqi}}</div>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('chuzhenyisheng','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('chuzhenyisheng','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('chuzhenyisheng','预约挂号')" 
							@click="onAcross('guahaoxinxi','','','','')" type="warning">
							预约挂号
						</el-button>
					</div>
				</div>
		...
		methods: {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/chuzhenyisheng', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/chuzhenyishengAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此出诊医生？') .then(_ => {
					this.$http.post('chuzhenyisheng/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
