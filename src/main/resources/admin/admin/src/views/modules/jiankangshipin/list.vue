<template>
	<div class="main-content" :style='{"padding":"30px 30px 20px 30px","margin":"30px 0 0 20px","fontSize":"15px","background":"#F2F8FF"}'>
		<!-- 列表页 -->
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 10px 0px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"20px 30px  10px 30px","alignSelf":"center","borderRadius":"30px 30px 0 0 ","background":"#fff","display":"flex"}' >
					<div :style='{"margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 5px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">视频名称</label>
						<el-input v-model="searchForm.shipinmingcheng" placeholder="视频名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 5px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">视频简介</label>
						<el-input v-model="searchForm.shipinjianjie" placeholder="视频简介" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"0","flexWrap":"wrap","background":"#fff","display":"flex"}'>
					<el-button class="add" v-if="isAuth('jiankangshipin','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"34px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('jiankangshipin','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"34px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('jiankangshipin','视频分类统计')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						视频分类统计
					</el-button>
					<el-button class="btn18" v-if="isAuth('jiankangshipin','视频创作统计')" type="success" @click="chartDialog2()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						视频创作统计
					</el-button>
				</el-row>
			...
			<div :style='{"width":"100%","padding":"0 10px","fontSize":"14px","color":"#000"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column  :resizable='true' prop="shipinfengmian" width="200" label="视频封面">
						<template slot-scope="scope">
							<div v-if="scope.row.shipinfengmian">
								<img v-if="scope.row.shipinfengmian.substring(0,4)=='http'&&scope.row.shipinfengmian.split(',w').length>1" :src="scope.row.shipinfengmian" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.shipinfengmian)">
								<img v-else-if="scope.row.shipinfengmian.substring(0,4)=='http'" :src="scope.row.shipinfengmian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.shipinfengmian.split(',')[0])">
								<img v-else :src="$base.url+scope.row.shipinfengmian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView($base.url+scope.row.shipinfengmian.split(',')[0])">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column :resizable='true' prop="shipinbofang" label="视频播放">
						<template slot-scope="scope">
							<el-button v-if="scope.row.shipinbofang" type="text" size="small" @click="preClick(scope.row.shipinbofang)">预览</el-button>
							<span v-else >无</span>
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('jiankangshipin','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('jiankangshipin','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								修改
							</el-button>
							...
							<el-button class="del" v-if="isAuth('jiankangshipin','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								删除
							</el-button>
						</template>
		...
		methods: {
			...
			chartDialog1() {
				...
				this.$nextTick(()=>{
					...
					this.$http({
						url: "jiankangshipin/group/shipinleixing",
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
			...
			getDataList() {
				...
				if(this.searchForm.shipinleixing!='' && this.searchForm.shipinleixing!=undefined){
 				...
 				if(this.searchForm.shipinjianjie!='' && this.searchForm.shipinjianjie!=undefined){
					params['shipinjianjie'] = '%' + this.searchForm.shipinjianjie + '%'
				}
