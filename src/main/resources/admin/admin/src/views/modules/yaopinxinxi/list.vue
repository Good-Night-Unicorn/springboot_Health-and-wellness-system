		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 10px 0px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"20px 30px  10px 30px","alignSelf":"center","borderRadius":"30px 30px 0 0 ","background":"#fff","display":"flex"}' >
					<div :style='{"margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 5px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">药品名称</label>
						<el-input v-model="searchForm.yaopinmingcheng" placeholder="药品名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","display":"flex"}' class="select" label="剂型" prop="jixing">
						<label :style='{"margin":"0 5px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">剂型</label>
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"0","flexWrap":"wrap","background":"#fff","display":"flex"}'>
					...
					<el-button class="btn18" v-if="isAuth('yaopinxinxi','库存统计')" type="success" @click="chartDialog1()">
			...
			<div :style='{"width":"100%","padding":"0 10px","fontSize":"14px","color":"#000"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column  :resizable='true' prop="yaopintupian" width="200" label="药品图片">
						<template slot-scope="scope">
							<div v-if="scope.row.yaopintupian">
								<img v-if="scope.row.yaopintupian.substring(0,4)=='http'&&scope.row.yaopintupian.split(',w').length>1" :src="scope.row.yaopintupian" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.yaopintupian)">
								<img v-else-if="scope.row.yaopintupian.substring(0,4)=='http'" :src="scope.row.yaopintupian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.yaopintupian.split(',')[0])">
								<img v-else :src="$base.url+scope.row.yaopintupian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView($base.url+scope.row.yaopintupian.split(',')[0])">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
		...
		methods: {
			...
			chartDialog1() {
				...
				this.$nextTick(()=>{
					...
					this.$http({
						url: `yaopinxinxi/value/yaopinmingcheng/shuliang`,
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
